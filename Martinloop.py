...
class Martinloop:
    def __init__(self, nsteps, smoothfac=3,center=True, normunit=False):
        ...
        if smoothfac>1:
            self.stepx = np.array([np.mean(self.stepx[[j % nsteps for j in range(l, l + smoothfac)]]) for l in range(0, nsteps)])
            self.stepy = np.array([np.mean(self.stepy[[j % nsteps for j in range(l, l + smoothfac)]]) for l in range(0, nsteps)])
        ...
        cxlim=(-tlim,tlim)
        cylim=(-tlim,tlim)
        self.nudgesmat=((smoothfac>(((np.arange(0,nsteps))+np.arange(nsteps,0,-1)[:,None])%nsteps)))/smoothfac
    ...
    def calcwalk(self,afc,nudge=0,center=False):
        ...
        wmat=np.minimum(1,np.maximum(0,afc-(((np.arange(0,self.nsteps))+np.arange(self.nsteps,0,-1)[:,None])%self.nsteps)))
